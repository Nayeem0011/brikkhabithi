const StickyCallButton = () => {
  return (
    <div className="fixed bottom-6 right-6 z-50 sm:bottom-8 sm:right-8">
      {/* Pulse ring effect */}
      <span className="absolute inset-0 rounded-full bg-plum/50 animate-ping" />
      <a
        href="https://wa.me/8801744483744"
        target="_blank"
        rel="noopener noreferrer"
        className="group relative flex items-center gap-2 rounded-full bg-plum px-4 py-3.5 font-semibold text-cream shadow-lg shadow-plum/40 transition-all duration-300 hover:scale-105 hover:shadow-xl hover:shadow-plum/50 sm:px-5"
      >
        <svg
          viewBox="0 0 24 24"
          className="h-6 w-6 shrink-0 fill-current transition-transform duration-300 group-hover:rotate-12 sm:h-5 sm:w-5"
        >
        ...
        <span className="hidden sm:inline">যোগাযোগ করতে</span>
      </a>
    </div>
  );
};
